...
public class PacketHaveRegionsC2S extends PacketGroup<PacketHaveRegionC2S> {
    ...
    public PacketHaveRegionsC2S(String server, String world, String dimension, List<PacketHaveRegionC2S> regions) {
        this.server = server;
        this.world = world;
    ...
    public ByteBuffer writePacket() {
        ...
        ByteBuffer buff = ByteBuffer.allocate(1 + Integer.BYTES * 3 + server.length + world.length + dimension.length + (Integer.BYTES * 2 + Long.BYTES) * children.size());
        buff.put(OPCODE);
    ...
    public PacketHaveRegionC2S createChild(ByteBuffer buff) {
        return new PacketHaveRegionC2S(server, world, dimension, buff);
    }
