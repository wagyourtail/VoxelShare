...
public class PacketPositionC2S extends Packet {
    ...
    public PacketPositionC2S(String server, String world, String dimension, int x, int z) {
        this.server = server;
        this.world = world;
    ...
    public ByteBuffer writePacket() {
        ...
        ByteBuffer buff = ByteBuffer.allocate(world.length + server.length + dimension.length + Integer.BYTES * 5 + 1);
        buff.put(OPCODE);
